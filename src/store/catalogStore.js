...
const catalogStore = {
  ...
  state: {
    catalogList: [
      ...
      },
      {
        ...
        raiting: 4.5,
        thumb:
          "https://cdn.mskguru.ru/uploads/news/parking-v-novostrojjke-vidyy-i-normyy_l.jpg",
        date: Date.now(),
      },
    ],
    /*  
  ...
  getters: {
    ...
    getCatalogListByDeveloper: (state) => (developer) => {
      return state.catalogList.filter(
        (c) => c.developer.toLowerCase().indexOf(developer.toLowerCase()) >= 0
      );
    },
    getCatalogListByTitle: (state) => (title) => {
      return state.catalogList.filter(
        (c) => c.title.toLowerCase().indexOf(title.toLowerCase()) >= 0
      );
    },
  ...
  mutations: {
    setSearchResultCatalogLis(state, list) {
      if (list.length > 0) {
        ...
        state.searchResultCatalogList = [
          { result: "Ничего не найдено", relation: "" },
        ];
      }
    },
  },
  actions: {
    fetchSearch({ commit, getters }, searchText) {
     // const response = await jwtInterceptor.get("http://localhost:3000/todos");
